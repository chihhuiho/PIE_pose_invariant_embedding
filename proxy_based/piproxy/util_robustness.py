...
class VGG_avg_proxy(nn.Module):
    def __init__(self, input_view, output_class):
        super(VGG_avg_proxy, self).__init__()
    ...
    def forward(self, x, sampled_view):
        ...
        if sampled_view == 1:
            ...
            for view in sampled_image:
                view_feature = self.features(x[:,view]).view(x.shape[0], 25088)            
                view_feature = self.classifier1(view_feature)
                
                if image_features is None:
                    image_features = view_feature.view(x.shape[0],1,4096)
                else:
                    image_features = torch.cat([image_features, view_feature.view(x.shape[0],1,4096)],1)
            ...
            return shape_feature, class_feature      