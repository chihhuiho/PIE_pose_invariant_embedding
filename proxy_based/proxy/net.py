...
from util import VGG_avg_proxy, compute_acc, mvDataset, load_data
...
def main():
    ...
    if arg.net == 'VGG_avg':
        model = VGG_avg_proxy(input_view, output_class)
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (batch_x, batch_y) in enumerate(dataloader_trn):
            ...
            image_feature, class_feature = model(input)
            batch_y = batch_y.view(batch_y.shape[0],1).repeat(1,input_view).view(batch_y.shape[0]*input_view)
            y = batch_y.numpy()
   
            loss = None
            for b in range(y.shape[0]):
                loss_image_nominator, loss_image_denominator = None, None
                for o in range(output_class):
                    
                    neg_dist = -torch.sum((image_feature[b]-class_feature[o])**2)
                    exp_neg_dist = torch.clamp(torch.exp(neg_dist), min=1e-16)
                    if o == y[b]:
                        loss_image_nominator = exp_neg_dist
                    else:
                        if loss_image_denominator is None:
                            loss_image_denominator = exp_neg_dist
                        else:
                            loss_image_denominator += exp_neg_dist
                            
                  
                loss_image = -(torch.log(loss_image_nominator) - torch.log(loss_image_denominator))
            ...
            if batch_idx%10==0:
                accuracy = compute_acc(image_feature, class_feature, y)*1.0/y.shape[0]
