...
from util import VGG_avg_pitc, compute_acc, mvDataset, load_data
...
parser.add_argument('--margin', action='store', default=1.0, type=float, help='margin (default: 1.0)')
...
def main():
    ...
    ch = logging.FileHandler('log/logfile_'+arg.net+'_'+arg.dataset + '_' + str(arg.margin)+ '_' + str(arg.alpha)+ '_' + str(arg.beta) + '_' + str(arg.trial)  + '.log')
    ...
    if arg.train_f:
        ...
        dataloader_val = DataLoader(dataset_val, batch_size=arg.batchSize, shuffle=False, num_workers=4)
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (batch_x, batch_y) in enumerate(dataloader_trn):
            ...
            for v in range(image_feature.shape[0]):
                min_dis, min_dist_v_s, min_dist_s_c , dist_v_sy, dist_s_cy = None, None, None, None, None
                shape_id = int(v/input_view)
                
                for s in range(shape_feature.shape[0]):
                    if shape_id == s:
                        dist_v_sy = torch.sum((image_feature[v]-shape_feature[s])**2)
                        dist_s_cy = torch.sum((shape_feature[s]-class_feature[y[v]])**2)
                        
                        for o in range(output_class):
                            if not o == y[v]:
                                dist_s_c = torch.sum((shape_feature[s]-class_feature[o])**2)
                                if min_dist_s_c is None:
                                    min_dist_s_c = dist_s_c
                                else:
                                    min_dist_s_c = torch.min(min_dist_s_c, dist_s_c)   
                    else:
                        dist_v_s = torch.sum((image_feature[v]-shape_feature[s])**2)
                        if min_dist_v_s is None:
                            min_dist_v_s = dist_v_s
                        else:
                            min_dist_v_s = torch.min(min_dist_v_s, dist_v_s)  
                            
                if loss is None:
                    loss = torch.clamp(arg.alpha*dist_v_sy + arg.beta*dist_s_cy - arg.alpha*min_dist_v_s - arg.beta*min_dist_s_c + arg.margin, min=0)        
                else:
                    loss += torch.clamp(arg.alpha*dist_v_sy + arg.beta*dist_s_cy - arg.alpha*min_dist_v_s - arg.beta*min_dist_s_c + arg.margin, min=0) 
            ...
            if batch_idx%10==0:
                accuracy = compute_acc(image_feature, class_feature, y)/y.shape[0]
