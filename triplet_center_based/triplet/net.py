...
from util import VGG_avg_tc, compute_acc, mvDataset, load_data
...
def main():
    ...
    if arg.net == 'VGG_avg':
        model = VGG_avg_tc(input_view, output_class)
    ...
    # for cpu mode
        ...
        model
    model_path = 'model/model_'+arg.net+'_'+arg.dataset+ '_' + str(arg.margin)  + '_' + str(arg.trial)  +'.pt'
    # training
        ...
        for batch_idx, (batch_x, batch_y) in enumerate(dataloader_trn):
            ...
            for b in range(y.shape[0]):
                ...
                for o in range(output_class):
                    if not o == y[b]:
                        dist = torch.sum((image_feature[b]-class_feature[o])**2)
                        if min_dis is None:
                            min_dis = dist
                        else:
                            min_dis = torch.min(min_dis, dist)
                          
                dist_y = torch.sum((image_feature[b]-class_feature[y[b]])**2)
